...
const createAdrFullPath = (directory : vscode.Uri, adrTitle: string, todayDate: string) => vscode.Uri.joinPath(directory,'ADR_'+adrTitle+'_'+todayDate+'.md');
const createAdrFile = (adrFullPath: vscode.Uri) => vscode.workspace.fs.writeFile(adrFullPath, Buffer.from(adrStrings.template));

const askForTitle = () => vscode.window.showInputBox({
...
export function activate(context: vscode.ExtensionContext) {
	...
	vscode.commands.registerCommand('adrutilities.adr', () => {
		...
		vscode.window.showInformationMessage('Hello World from adrutilities! Eh ouais !');
	});

	vscode.commands.registerCommand('adrutilities.create.adr', async ()=> {
		...
		let uri = await parentDirectory;
		if(!!uri){
			let adrTitle = await askForTitle();
			if(!!adrTitle){
				let today = concatDate();
				let fullPath = createAdrFullPath(uri[0],adrTitle,today);
