...
suite("ADR CodeLens Navigation Provider Test Suite", () => {
  ...
  test("SecurityValidator integration in provider should work", () => {
    ...
    assert.strictEqual(
      SecurityValidator.escapeRegex("adr.*+?^${}()|[\\]"),
      "adr\\.\\*\\+\\?\\^\\$\\{\\}\\(\\)\\|\\[\\\\\\]",
    );
  ...
  test("should provide code lenses for ADR files in markdown links without whitespace", async () => {
    const provider = new AdrCodelensNavigationProvider();
    const lineText = "[My ADR](adr_MyADR_20260720.md).";
    const adrReference = "adr_MyADR_20260720.md";
    ...
    const mockDocument = {
      ...
      lineAt: (line: number) => ({ text: lineText, lineNumber: line }),
      ...
      getWordRangeAtPosition: (position: vscode.Position, regex?: RegExp) => {
        ...
        const wordRegex = new RegExp(regex.source, regex.flags);
    ...
    try {
      ...
      assert.strictEqual(
        result.length,
        1,
        "Should detect exactly one ADR reference in markdown link (in an array)",
      );
      console.log(result);
      const matchedText = mockDocument.getText(result[0].range);
      assert.strictEqual(
        matchedText,
        adrReference,
        "CodeLens range should match ADR filename without markdown punctuation",
      );
    } finally {
